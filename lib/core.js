    ...
    if (_ship_data.priorities && _ship_data.priorities.length > 0) {
      ...
      for (var i = 0; i < _ship_data.priorities.length; i++) {
        ...
        if (damage_transfer > 0) {
          ...
          damage += battleDamageReverse(_ship_data, _defender_data, damage_transfer);
        }

        alreadyDamaged.push(_ship_data.priorities[i]);
      }
    }
...
function getAugmentations(unit_id, config) {
  // Базовые значения
  console.log(CJC.data.humans[unit_id], config);
  ...
  console.log('percentAttack', percentAttack);
