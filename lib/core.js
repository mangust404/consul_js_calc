...
function getAugmentations(unit_id, config) {
  ...
  if (config.bonusItemsHP > 0) {
    percentHP += parseFloat(config.bonusItemsHP) + parseFloat(config.bonusItemsHP) * parseInt(config.energyLvl) / 100;
  }

  // Бонусы палаты на урон (тронный зал жестокости)
  if (config.bonusItemsAttack > 0) {
    percentAttack += parseFloat(config.bonusItemsAttack) + parseFloat(config.bonusItemsAttack) * parseInt(config.energyLvl) / 100;
  }
  ...
  if (unit_id == 'flagship') {
    percentAttack += parseFloat(config.doomDayCalibrationLvl);
