...
QUnit.test( "battleRound ударный флот 2.2.2 с советом", function( assert ) {
  var humans = {
    dreadnought: {
      count: 2,
      damage: 0,
  ...
  assert.equal(log1.reptiles.hydra.hp, 585025.5)
  assert.equal(log1.reptiles.armadillo.hp, 60956)
  assert.equal(log1.reptiles.godzilla.hp, 2096146)

  // Следующий раунд
  var log2 = CJC.__call('battleRound', [humans, reptiles]);
  console.log(log2);

  assert.equal(log2.reptiles.hydra.hp, 216164.5)
  assert.equal(log2.reptiles.armadillo.hp, 0)
  assert.equal(log2.reptiles.godzilla.hp, 753248)

  // Следующий раунд
  var log3 = CJC.__call('battleRound', [humans, reptiles]);
  console.log(log3);

  assert.equal(log3.reptiles.hydra.hp, 0)
  assert.equal(log3.reptiles.armadillo.hp, 0)
  assert.equal(log3.reptiles.godzilla.hp, 0)
});
